...
export function CardsSummaryTable({
  ...
  if (cards.length === 0) {
    return (
      <Paper variant="outlined" sx={{ height: '100%', p: 4, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    <Box>
      ...
      {hasSiblings && (
        <Collapse in={expanded}>
          {siblingCards.map((sibling) => {return (
              <Box
                key={sibling.id}
                sx={{
                  px: 2,
