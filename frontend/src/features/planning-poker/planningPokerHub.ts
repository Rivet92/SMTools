...
export const {
  ...
  registerHandlers: (connection) => {
    ...
    connection.on('VoteRevealed', createDeltaHandler(connection, (p: VoteRevealedPayload) => {
      usePlanningPokerStore.getState().applyVoteRevealed(p);
    }));

    connection.on('VoteItemAdded', createDeltaHandler(connection, (p: VoteItemAddedPayload) => {
      usePlanningPokerStore.getState().applyVoteItemAdded(p);
    }));

    connection.on('VoteItemDeleted', createDeltaHandler(connection, (p: VoteItemDeletedPayload) => {
      usePlanningPokerStore.getState().applyVoteItemDeleted(p);
    }));

    connection.on('VotesReset', createDeltaHandler(connection, (p: VotesResetPayload) => {
...
export const revealVotes = (voteItemId: string) =>
  hubInvoke<void>('RevealVotes', voteItemId);
...
export const focusVoteItem = (voteItemId: string) =>
  hubInvoke<void>('FocusVoteItem', voteItemId);
