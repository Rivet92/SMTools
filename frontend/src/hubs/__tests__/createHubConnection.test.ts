...
describe('createHubConnection', () => {
  ...
  it('creates a connection with the given URL', () => {
    const hub = createHubConnection('/hubs/test', {}, createActions(), () => undefined, () => undefined, vi.fn(), () => null);
  ...
  it('ensureConnected starts the connection if disconnected', async () => {
    ...
    const hub = createHubConnection('/hubs/test', {}, actions, () => undefined, () => undefined, vi.fn(), () => null);
  ...
  it('registerHandlers is called with the connection', async () => {
    ...
    const hub = createHubConnection('/hubs/test', listeners, createActions(), () => undefined, () => undefined, vi.fn(), () => null);
    await hub.ensureConnected();
  ...
  it('registers RoomUpdated handler that calls setRoom', async () => {
    const setRoom = vi.fn();
    const listeners = createListeners();
    const hub = createHubConnection('/hubs/test', listeners, createActions(), () => undefined, () => undefined, setRoom, () => null);
    await hub.ensureConnected();

    const handler = mockConnection.on.mock.calls.find((call: unknown[]) => call[0] === 'RoomUpdated')?.[1];
  ...
  it('registers RoomClosed handler', async () => {
    ...
    const handler = mockConnection.on.mock.calls.find((call: unknown[]) => call[0] === 'RoomClosed')?.[1];
    expect(handler).toBeDefined();
  });
  ...
  it('registers YouWereRemoved handler', async () => {
    ...
    const handler = mockConnection.on.mock.calls.find((call: unknown[]) => call[0] === 'YouWereRemoved')?.[1];
    expect(handler).toBeDefined();
  });
});
